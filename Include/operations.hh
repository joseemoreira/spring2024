...
namespace CDC8600
{
	...
	class operation
	{
		 ...
		 {
		     _count = PROC.op_count;				// current instruction count
	...
	{
	    public:
		vector<units::unit>& units() { return PROC.FXUs; }
	};

	class FPop : public operation
	{
	    public:
		vector<units::unit>& units() { return PROC.FPUs; }
	};

	class LDop : public operation
	{
	    public:
		vector<units::unit>& units() { return PROC.LDUs; }
	};

	class STop : public operation
	{
	    public:
		vector<units::unit>& units() { return PROC.STUs; }
	};

	class BRop : public operation
	{
	    public:
		vector<units::unit>& units() { return PROC.BRUs; }
	};
	...
	class xkj : public FXop
	{
		...
		u64 target(u64 cycle) { PROC.REGready[_j] = cycle; }
	...
	class idzkj : public FXop
	{
		...
		u64 ready() const { return PROC.REGready[_k]; }
		u64 target(u64 cycle) { PROC.REGready[_j] = cycle; }
		u64 latency() const { return 2; }
		u64 throughput() const { return 1; }
	...
	class idjkj : public FXop
	{
		...
		u64 ready() const { return PROC.REGready[_j]; }
		u64 target(u64 cycle) { PROC.REGready[_j] = cycle; }
		u64 latency() const { return 2; }
		u64 throughput() const { return 1; }
        ...
        class idjki : public FXop
	{
		...
		u64 ready() const { return max(PROC.REGready[_k], PROC.REGready[_j]); }
		u64 target(u64 cycle) { PROC.REGready[_i] = cycle; }
		u64 latency() const { return 2; }
		u64 throughput() const { return 1; }
	...
	class agen : public FXop
	{
		...
		u64 ready() const { return max(max(PROC.REGready[_k], PROC.REGready[_j]), max(PROC.REGready[params::micro::RA], PROC.REGready[params::micro::FL])); }
		u64 target(u64 cycle) { PROC.REGready[_i] = cycle; }
		u64 latency() const { return 2; }
	...
	class rdw : public LDop
	{
		...
		u64 ready() const { return max(PROC.REGready[_k], MEMready[_addr]); }
		u64 target(u64 cycle) { PROC.REGready[_j] = cycle; }
		u64 latency() const { return PROC.L1D.loadhit(_addr, _issue) ? params::L1::latency : params::MEM::latency; }
		u64 throughput() const { return 1; }
	...
	class stw : public STop
	{
		...
		u64 latency() const { return PROC.L1D.storehit(_addr, _issue) ? params::L1::latency : params::MEM::latency; }
		u64 throughput() const { return 1; }
	...
	class cmpz : public FXop
	{
		...
		cmpz(u08 j) { _j = j; }
		u64 ready() const { return PROC.REGready[_j]; }
		u64 target(u64 cycle) { PROC.REGready[params::micro::CMPFLAGS] = cycle; }
	...
	class cmp : public FXop
	{
		...
		u64 ready() const { return max(PROC.REGready[_j], PROC.REGready[_k]); }
		u64 target(u64 cycle) { PROC.REGready[params::micro::CMPFLAGS] = cycle; }
		u64 latency() const { return 1; }
		u64 throughput() const { return 1; }
	...
	class jmp : public BRop
	{
		...
		jmp() { }
		u64 ready() const { return 0; }
		u64 target(u64 cycle) { PROC.op_nextdispatch = cycle; }
		u64 latency() const { return 1; }
		u64 throughput() const { return 1; }
	...
	class jmpz : public BRop
	{
	    ...
	    public:
		jmpz() { }
		u64 ready() const { return PROC.REGready[params::micro::CMPFLAGS]; }
		u64 target(u64 cycle) { PROC.op_nextdispatch = cycle; }
		u64 latency() const { return 1; }
		u64 throughput() const { return 1; }
